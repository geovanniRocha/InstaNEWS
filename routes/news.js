...
router.get('/:tk', function(req, res, next) {
  ...
  knex.select(["news.Title", "news.description", 'news.url']).from('users').where("users.token", id)
  ...
  .innerJoin('news', 'categories.idcategories','news.feed_idfeed') 
  .timeout(1000)
...
router.get('/', function(req, res, next) {
  ...
  knex.select().from('news').timeout(1000).then(rSet =>{
    res.send(rSet);
  }) 
  return 200;
 // res.send(resp);
